...
if(userinfo.email && userinfo.password){
	...
	//"provider":{"$ne":["F","G"]
	userModel.getOne({"email":userinfo.email,"password":userinfo.password},function(err,result){
if(err){
	res.status(500);
...
if(!(validator.isNull(resetData.password))){
 ...
 userModel.getOne({"_id":resetData._id},function(err,result){
	if(err){
		...
		userModel.update({"_id":result._id},{"password":md5(resetData.password),"preset":true},function(err1,result1){
			if(err1){
				res.status(500);
...
loginAuditInfo.datetime = moment.utc().format();// 'YYYY-MM-DDTHH:mm:ss'
 loginAudit.insert(loginAuditInfo,function(err,result){
if(err){
	res.status(500);
