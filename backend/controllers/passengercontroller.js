...
if(matchErr){
 ...
 res.send({"results":[]});
  ...
  //    sendError(errMsg,"Error while getting the directions",res);
		...
		async.parallel({
			start:function(startCallback){
				db.collection("match_share").find({"startLocation" : { $nearSphere : {$geometry: matchData.startLocation, $maxDistance: (distance * 1000) }}},{"trip_details":false,"Routes":false}).toArray(startCallback);
			},
			end:function(endCallback){
				db.collection("match_share").find({"endLocation": { $nearSphere : {$geometry: matchData.endLocation, $maxDistance: (distance * 1000) }}},{"trip_details":false,"Routes":false}).toArray(endCallback);
			}
		},function(errTrip,matchResult){
			console.log(matchResult);
			if(errTrip){
			 ...
			 // res.send({"status":"error","msg":"Error while getting the trip data"});
				...
				async.eachSeries(matchids,function(eachMatch_id,tripCallback){
				 ...
				 if(matchObj){
					...
					db.collection("user").findOne({"_id":matchObj.user_id},{"_id":true,"provider_id":true,"username":true,"email":true,"phonenumber":true,"image_url":true},function(err,userResult){
						if(err){
							tripCallback(err);
						}else{
							//matchObj.userResult = userResult;
